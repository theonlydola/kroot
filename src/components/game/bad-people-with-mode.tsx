...
export function BadPeopleWithMode({
  cards,
  dict,
  onlineDict,
  slug,
  lang,
}: BadPeopleWithModeProps) {
  ...
  const handleCreateRoom = useCallback(async () => {
    ...
    const config: BadPeopleGameConfig = {
      maxPlayers,
    };
  ...
  }, [hostName, maxPlayers, slug, lang, router]);
  ...
  if (mode === "local") {
    return (
      <BadPeopleGame
        cards={cards}
        dict={dict as Parameters<typeof BadPeopleGame>[0]["dict"]}
        slug={slug}
        lang={lang}
      />
    );
